...
- **readers borrow `&self`** (`get_atom`, `get_observations_by_subject`, …) and can run
  concurrently;
...
The Lance store exposes two layers: **trait methods** (defined in
[`observatory-store`], implemented here) that speak only domain types, and
**inherent methods** specific to Lance that speak Lance types (lifecycle and
maintenance). Code that needs the Lance-specific surface holds the concrete
type (`LanceAtomStore` / `LanceObservationStore`); code that is
backend-agnostic holds the trait.

Async signatures (illustrative — the design, not the implementation):

```rust
// ── trait surface (in observatory-store, implemented here) ───────────────
#[async_trait]
impl AtomStore for LanceAtomStore {
    // Streaming-first: the caller feeds a Stream<Item = Atom>, the impl
    // drains it in CHUNK_SIZE chunks, one logical commit at the end.
    async fn put_atoms(&mut self, atoms: impl Stream<Item = Atom> + Unpin + Send + 'static) -> Result<()>;
...
impl ObservationStore for LanceObservationStore {
    async fn put_observations(&mut self, observations: impl Stream<Item = Observation> + Unpin + Send + 'static) -> Result<()>;
    async fn get_observation_by_id(&self, id: ObservationId) -> Result<Option<Observation>>;  // point lookup (id is unique)
    async fn get_observations_of_kind(&self, kind: &Kind) -> Result<Vec<Observation>>;        // scan filter on kind
    async fn get_observations_by_subject(&self, atom: AtomId) -> Result<Vec<Observation>>;    // array-membership on subjects
}

// ── Lance-specific inherent surface (off-trait: speaks Lance types) ──────
impl LanceAtomStore {
    async fn open(uri: &str) -> Result<LanceAtomStore>;    // errors if absent
    async fn create(uri: &str) -> Result<LanceAtomStore>;   // empty dataset; errors if present
...
impl LanceObservationStore {
    async fn open(uri: &str) -> Result<LanceObservationStore>;
    async fn create(uri: &str) -> Result<LanceObservationStore>;
...
- `subjects` (observations) → **LABEL_LIST** — array-membership for `get_observations_by_subject` (the index's entire raison d'être).
...
### `get_observations_by_subject` and the `array_has` cast

The `get_observations_by_subject` predicate is `array_has(subjects, arrow_cast(X'<hex>',
...
3. **Indexes + maintenance (DONE).** `ensure_indexes` (BTREE on `atom_id`;
   ...
   `get_observations_by_subject` (array-membership via `array_has` on the LABEL_LIST
   ...
   dataset, and a proptest property for `get_observations_by_subject`.
...
- DuckDB read path and which integration (extension vs. Arrow handoff) —
  increment 4.
- **Trait distillation and the crate split — DONE.** The store trait(s) were
  _derived, not predicted_: both concrete Lance stores were built first, then
  the shared shape was distilled into `observatory-store` as two traits
  (`AtomStore` / `ObservationStore`) plus a single backend-agnostic
  `StoreError`. The Lance implementation moved into `observatory-lance`, which
  depends on the trait crate; `observatory-store` is now Lance-free. Trait
  signatures speak only domain types (`Atom` / `AtomId` / `Observation` /
  `ObservationId` / `Kind` / `StoreError`); Lance types surface only in
  impl-specific inherent methods that live _outside_ the trait (lifecycle,
  maintenance). Write methods are streaming-first (`Stream<Item = T>`) to
  accommodate incremental producers without materializing the whole batch.
  The `observatory-arrow` extraction (splitting the domain↔Arrow mapping
  into its own crate) remains deferred — the arrow-mapping modules already
  never import `lance`, so it is a near-mechanical refactor whenever a real
  second backend would want it. Lance is still the first-class citizen: the
  shared schema deliberately targets what Lance can store (the `Union`
  rejection is precisely this), so any future backend must support at least
  that.
